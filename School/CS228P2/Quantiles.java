...
public class Quantiles{
	...
	n = data.length;
	numq = q;
	int quant = (int)(Math.ceil(n/q));
	if( q < 1 || q > n){
	    System.out.println ("Inoperable quantile size.");}
	else{
	    quants = new int[q-1];
	    for(int i = 1;i < q; ++i){
		quants[i-1] = OrderStatistics.select(data,0,n-1,i*quant);
		System.out.println(quants[i-1]);}
	    for(int j = 0;j < quant;++j){
		bottomTotal += OrderStatistics.select(data,0,n-1,j);
		System.out.println(bottomTotal);}
	    for(int k = quant*(q-1);k+1<n;--k){
		topTotal += OrderStatistics.select(data,0,n-1,k);}}}
	...
	String endgame = n + " " + numq + " " + quants + " " + topTotal + " " + bottomTotal + "\n";
	return endgame;}}
